...
def get_loaders(train_ids, valid_ids, bs=32, num_workers=4, preprocessing_fn=None,
...
if __name__ == "__main__":
    ...
    logdir_base = f"./logs/{arch}_{encoder}"

    train_ids, valid_ids = get_ids()
    all_ids = list(train_ids) + list(valid_ids)
    N_FOLDS = 5
    splits = np.array_split(all_ids, N_FOLDS)
    ...
    for i in range(N_FOLDS):
        ...
        print(f"Fold: {i}")
        logdir = logdir_base+f"_{i}"
        test_ids = splits[i]
        train_ids = np.array(list(set(all_ids) - set(test_ids)))
        model, preprocessing_fn = get_model(encoder, type=arch)
        if use_wandb:
            wandb.watch(model)
        loaders = get_loaders(train_ids, valid_ids, bs, num_workers, preprocessing_fn)
